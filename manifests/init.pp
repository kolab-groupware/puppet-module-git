...
class git {
    ...
    define repository(  $public = false, $shared = false,
        ...
        realize(File["/usr/local/bin/git_init_script"])
    ...
    define clean(
        ...
        exec { "git_clean_exec_$name":
            ...
            command => $excluded ? {
                    true => "git clean -d -f -x",
                    default => "git clean -d -f"
                }
        }
    }
    ...
    define reset(
        ...
        exec { "git_reset_exec_$name":
            ...
            command => $branch ? {
                    ...
                    default => "git reset --hard origin/$branch"
                }
        }
    ...
    define clone(   $source,
        ...
        exec { "git_clone_exec_$localtree/$_name":
            cwd => $localtree,
            command => $mirror ? {
                    true => "git clone `echo $source | sed -r -e 's,(git://|ssh://)(.*)//(.*),\\1\\2/\\3,g'` $_name",
                    default => "git clone `echo $source | sed -r -e 's,(git://|ssh://)(.*)//(.*),\\1\\2/\\3,g'` $_name"
                },
            creates =>  $mirror ? {
                    true => "$localtree/$_name/HEAD",
                    default => "$localtree/$_name/.git/"
                },
